...
class _FnMaterialBI:
    ...
    def __same_image_file(self, image, filepath):
        if image and image.get('mmd_tools_missing_filepath') == filepath:
            return True
        ...
        if img is None:
            if os.path.exists(filepath):
                try:
                    ...
                    logging.warning('Cannot load texture %s. A placeholder will be used.', filepath)
            else:
            ...
            if img is None:
                ...
                img['mmd_tools_missing_filepath'] = filepath
        ...
        if sphere_texture_type not in (1, 2, 3):
            ...
            if texture and (not texture.inputs['Vector'].is_linked or texture.inputs['Vector'].links[0].from_node.name == 'mmd_tex_uv'):
                if hasattr(texture, 'color_space'):
                    ...
                    colorspace = texture.image.colorspace_settings
                    if is_sph_add:
                        for name in ('Linear Rec.709', 'Linear', 'Non-Color'):
                            try:
